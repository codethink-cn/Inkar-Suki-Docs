...
`Inkar-Suki`自带一套较为完善的权限系统，该权限系统分为`1-10`，共`12`个等级，每个等级对应的特权不同，但高权限包含低权限。

请在`Inkar-Suki/src/tools`中创建一个名为`permission.json`的文件，内容为`{"您的QQ号":"10"}`，如果有多个，可以按`json`语法自行添加，也可以通过机器人指令进行添加。

!> `10`级权限具有操控后台的能力，该功能被设计用于方便开发者，但是谨防提权，小心`SSH`反射等行为，慎给`10`级权限！<br>
> 只有同时具有`10`级权限以及处于配置文件中`owner`的列表中的用户具有给予`10`级权限的能力。<br>

!> 如果没有配置`permission.json`，启动时可能会报错，内容大致是找不到文件。
