...
>
> 如果是`[a|b>`参数，表示此处如果是`a`参数可以省略，如果是`b`参数则为必选。
>
> 除特殊声明外，参数中的`|`表示二选一，`/`就是`/`，作为一个字符。
>
> 权限等级中的`群`表示`群主或管理员`。
...
|命令|格式|别名|描述|权限|
|-----|-----|-----|-----|-----|
|`+arcuser`|`+arcuser <usercode\|username>`|`-`|`查询用户信息和最近一次游玩记录。`|`-`|
|`+arcbind`|`+arcbind <usercode\|username>`|`-`|`在群内绑定自己的Arcaea账号。`|`-`|
|`+arcbest`|`+arcbest <songname> <difficulty>`|`-`|`查询个人单曲最佳成绩。`|`-`|
|`+arcunbind`|`+arcunbind`|`-`|`在群内取消绑定自己的Arcaea账号。`|`-`|
